...
const DuplicateDonor = () => {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const response = await apiClient.get(`${DUPLICATE_LIST}?${params}`);
  ...
  const handleDuplicateDelete = async () => {
    ...
    try {
      ...
      toast.error(
        error.response.data.message || "Error while deleting donor duplicate"
      );
    } finally {
