...
const DuplicateDonorEdit = () => {
  ...
  const { data: donors = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await apiClient.get(
        `/api/donor-duplicate-change?indicomp_full_name=${donor.indicomp_full_name}&indicomp_mobile_phone=${donor.indicomp_mobile_phone}`
      );
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message || "Duplicate Update Error Occurred."
      );
    } finally {
              ...
              <form onSubmit={onSubmit} className="space-y-3">
                ...
                {selectedDonorId && (
                  <div className="p-2 bg-green-50 border border-green-200 rounded text-xs text-green-800 font-medium">
                    Selected:{" "}
                    {
                      donors.find((d) => d.indicomp_fts_id === selectedDonorId)
                        ?.indicomp_full_name
                    }
                  </div>
                )}
...
const CompactDonorSelectTable = ({
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const token = Cookies.get("token");
      const response = await apiClient.get(
        `/api/donor-duplicate-change?indicomp_full_name=${encodeURIComponent(
          donorName
        )}&indicomp_mobile_phone=${encodeURIComponent(donorPhone)}`
      );
      return response.data?.data || [];
    },
  ...
  const columns = [
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="text-xs font-medium text-gray-600">{globalIndex}</div>
        );
      },
      size: 50,
    },
    ...
    },
    {
      ...
      header: "Mobile",
      cell: ({ row }) => (
        <div className="text-xs">{row.getValue("Mobile")}</div>
      ),
      size: 110,
    },
    {
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <Button
          variant={
            selectedDonorId === row.original.indicomp_fts_id
              ? "default"
              : "outline"
          }
          size="sm"
          ...
          <TableBody>
            {isFetching && !table.getRowModel().rows.length ? (
              ...
              <TableRow>
                ...
                >
                  <div className="text-gray-500 text-xs">No donors found</div>
                </TableCell>
              </TableRow>
            )}
          </TableBody>
