...
const fetchDonorById = async (id) => {
  const { data } = await apiClient.get(`/api/donor-view/${id}`);
  return data;
};

const fetchOldReceipts = async (id) => {
  const { data } = await apiClient.get(`/api/fetch-receipts-by-old-id/${id}`);
  return data;
};

const fetchDonorReceipts = async (id) => {
  const { data } = await apiClient.get(`/api/donor-all-receipts/${id}`);
  return data;
};
      ...
      <div className="container mx-auto  space-y-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
          <StatCard
            ...
            description={`${
              statistics?.donationGrowth?.toFixed(1) || 0
            }% growth`}
            trend="up"
          />
          <StatCard
