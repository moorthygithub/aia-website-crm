...
const MemberDashboard = () => {
  ...
  const { data: dashboardData, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiClient.get(MEMBER_DASHBOARD);
  ...
  const sendBulkEmailMutation = useMutation({
    mutationFn: async (year) => {
      const membersForYear =
        dashboardData?.individualCompanies?.filter(
      ...
      const response = await apiClient.post(SEND_BULK_EMAIL, {
  ...
  const handleYearCardClick = (year) => {
    ...
    const serializedMembers = encodeURIComponent(
      btoa(JSON.stringify(membersForYear))
    );
  ...
  const handleSendEmail = (year, e) => {
    ...
    if (membersWithoutEmail.length > 0) {
      toast.error(
        "Some members do not have an email. Please add their emails first."
      );
      return;
    }
  ...
  const getEmailStats = (year) => {
    const stats = memberEmailStats.find(
      (stat) => stat.member_vailidity === year
    );
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <h3 className="text-lg font-semibold">
                {Object.keys(membersByYear).length}
              </h3>
            </div>
        ...
        <Card className="p-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <h3 className="text-lg font-semibold">
                {memberEmailStats.reduce(
                  (total, stat) => total + stat.notsent,
                  0
                )}
              </h3>
            </div>
          ...
          {filteredYears.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              {filteredYears.map((year) => {
                ...
                const yearExistsInMemberEmail =
                  Array.isArray(memberEmailStats) &&
                  memberEmailStats.some(
                    (stat) => String(stat.member_vailidity) === String(year)
                  );
