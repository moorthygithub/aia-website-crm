...
const Viewer = () => {
  ...
  const {
    ...
    queryKey: ["viewers"],
    queryFn: async () => {
      const response = await apiClient.get(`/api/fetch-user-list`);
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const response = await apiClient.get(`/api/chapter-active`);
      return response.data.data;
    },
    keepPreviousData: true,
    staleTime: 5 * 60 * 1000,
  ...
  const columns = [
    ...
    },
    {
      ...
      header: "Email",
      cell: ({ row }) => (
        <div className="text-xs text-blue-600">{row.getValue("Email")}</div>
      ),
      size: 200,
    },
    {
    ...
    },
    {
      ...
      header: "Position",
      cell: ({ row }) => (
        <div className="text-xs">{row.getValue("Position") || "Viewer"}</div>
      ),
      size: 120,
    },
    {
      ...
      cell: ({ row }) => (
        <div className="text-xs">
          {row.getValue("Start Date")
            ? new Date(row.getValue("Start Date")).toLocaleDateString()
            : "-"}
        </div>
      ),
      size: 120,
    },
    {
      ...
      cell: ({ row }) => (
        <div className="text-xs">
          {row.getValue("Last Date")
            ? new Date(row.getValue("Last Date")).toLocaleDateString()
            : "-"}
        </div>
      ),
      size: 120,
