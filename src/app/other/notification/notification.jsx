  ...
  const markAsReadMutation = useMutation({
    mutationFn: async (noticeId) => {
      const response = await apiClient.post(
        `${OTHER_NOTIFICATION_MARK_AS_READ}/${noticeId}/read`,
        {}
      );
  ...
  const editNoticeMutation = useMutation({
    mutationFn: async ({ id, noticeData }) => {
      const response = await apiClient.put(
        `${OTHER_NOTIFICATION_SUMBIT_NOTICE}/${id}`,
        noticeData
      );
      return response.data;
    },
                ...
                >
                  <div className="flex-1">
                    ...
                    <span className="text-xs text-gray-500 mt-1 block">
                      Posted on:{" "}
                      {moment(row.original.created_at).format("DD-MM-YY")}
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
                    {row.original.is_read === 0 ? (
    ...
    <DialogContent className="sm:max-w-md">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <Select
            value={formData.to_be_sent_to}
            onValueChange={(value) => handleChange("to_be_sent_to", value)}
          >
