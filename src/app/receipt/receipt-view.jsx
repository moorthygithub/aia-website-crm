...
const ReceiptOne = () => {
  ...
  const { data: receiptData, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await apiClient.get(`/api/receipt-view?id=${id}`);
      return data;
    },
    enabled: !!id,
  ...
  const { data: receiptControl } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiClient.get(`/api/fetch-receipt-control`);
  ...
  const sendEmailMutation = useMutation({
    mutationFn: async () => {
      const response = await apiClient.get(`/api/send-receipt-email?id=${id}`);
  ...
  const updateEmailMutation = useMutation({
    mutationFn: (formData) =>
      apiClient.patch(
        `/api/update-donor-email/${Cookies.get("ftsid")}`,
        formData
      ),
