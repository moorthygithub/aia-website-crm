...
const ReceiptEdit = () => {
  ...
  const { data: receiptData, isLoading: isLoadingReceipt } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiClient.get(`/api/receipt/${id}`);
  ...
  const isLoadingHook =
    isLoadingDatasource || isLoadingMembershipYear || isLoadingSchoolAllotYear;
    ...
    if (receiptData) {
      setReceipt({
        ...
        receipt_financial_year:
          receiptData.receipt_financial_year || currentYear,
    ...
    setReceipt((prevReceipt) => {
      ...
      if (stateName === "receipt_donation_type") {
        if (value === "Membership") {
          ...
          updatedReceipt.receipt_total_amount =
            prevReceipt.receipt_total_amount;
        }
      }
  ...
  const handleSubmit = async (e) => {
    ...
    const formData = {
      ...
      with_out_panno:
        !receipt.individual_company?.indicomp_pan_no ||
        receipt.individual_company?.indicomp_pan_no === "NA"
          ? "Yes"
          : "No",
    };
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">
                    ...
                    <span className="text-muted-foreground font-semibold">
                      {receipt.individual_company?.indicomp_full_name}
                    </span>{" "}
                    ({receipt.individual_company?.indicomp_fts_id})
                  </p>
                  <p className="text-xs text-gray-500">
                    Receipt Ref:{" "}
                    <span className="text-muted-foreground font-semibold">
                      {receipt.receipt_ref_no}
                    </span>
                  </p>
                </div>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-2 mt-2 pt-2 border-t border-gray-200 text-xs text-gray-600">
                ...
                {pan && (
                  <div className="flex items-center gap-1 bg-gray-50 px-2 py-1 rounded">
                    <Shield
                      className={`w-3 h-3 ${
                        pan ? "text-gray-500" : "text-red-600"
                      }`}
                    />
                    <span>PAN: {pan}</span>
                  </div>
                ...
                <div className="flex flex-wrap gap-1">
                  {(receipt.receipt_exemption_type === "80G"
                    ...
                    <Button
                      ...
                      type="button"
                      variant={
                        receipt.receipt_donation_type === option.value
                          ? "default"
                          : "outline"
                      }
                      size="sm"
                  ...
                  {paymentModeOptions
                    ...
                    .map((option) => (
                      <Button
                        ...
                        type="button"
                        variant={
                          receipt.receipt_tran_pay_mode === option.value
                            ? "default"
                            : "outline"
                        }
                        size="sm"
              ...
              {receipt.receipt_donation_type === "General" && (
                <div className="space-y-1">
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      setReceipt((prev) => ({ ...prev, donor_source: value }))
                    }
                  >
                    <SelectTrigger className="h-7 text-xs">
